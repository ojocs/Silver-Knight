...
function create() {
    ...
    ledge.body.immovable = true;ledge.scale.setTo(8, 1);
    ...
    giant.body.setSize(200, 520, 280, 170);
    ...
    //Debugging
    debug = game.add.text(1500, 16, 'Boss Health: 1000', {
...
function teleTimers(){
    ...
    } else if (!canTele && teleTimer < 250) {

        teleTimer += 1;

        timerSprite.frame = 0;

    } else if (!canTele && teleTimer < 500) {

        teleTimer += 1;

        timerSprite.frame = 1;

    } else if (!canTele && teleTimer < 750) {

        teleTimer += 1;

        timerSprite.frame = 2;      

    } else if (!canTele && teleTimer < 1000) {
    ...
    if (blinkTimer === 20 && blinkCount > 0) {//Still have all blinks
        canBlink = true;
    } else if(blinkTimer === 300 && blinkCount === 0) {//Used up all blinks
        canBlink = true;
        blinkCount = 3;
    }
}
...
function teleControls(){
    ...
    //Controls for blinking and teleporting
...
function giantSwing(bossOrientation){
    ...
    var x2L = [-60, -250, -200], x2R = [-240, -50, -90];
...
function giantStomp(touchingGround){
    ...
    stompTimer.start();
    if(giant.stomping && knight.body.touching.down)
        knightDamage();
}
...
function debugF(knightSlash){
    var distanceFromBoss = (giant.body.center.x - knight.body.center.x);
    debug.text = 'BossHealth '+bossHealth+'\nstomping'+giant.stomping+'\nturning'+giant.turning+'\ngiant.scale.x'+giant.scale.x+'\ngiant.newScaleX'+giant.newScaleX+'\ngiant.body.x'+giant.body.x;
